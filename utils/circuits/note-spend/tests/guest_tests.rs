...
// Current on-chain blacklist parameters (must match the guest program).
...
// =============================================================================
// Viewer Attestation Indexing (Note Spend V2)
//
// The viewer section is appended after blacklist args as:
//   n_viewers (PUBLIC)
//   For each viewer:
//     fvk_commit (PUBLIC)
//     fvk        (PRIVATE)
...
//       mac      (PUBLIC)
//
// This helper is intentionally argument-index-only (no witness construction), and is used to
// ensure we never accidentally mark `fvk` as public in host/runner integrations.
// =============================================================================

fn base_args_no_viewers_v2(depth: usize, n_in: usize, n_out: usize, withdraw_amount: u64) -> usize {
    let per_in = 5 + depth; // value,rho,sender_id,pos,siblings[depth],nullifier
    let bl_checks = if withdraw_amount == 0 { 2 } else { 1 };
    6 + n_in * per_in + 3 + 5 * n_out + 1 + 1 + bl_checks * (BL_BUCKET_SIZE + 1 + BL_DEPTH)
}

fn viewer_start_index_v2(depth: usize, n_in: usize, n_out: usize, withdraw_amount: u64) -> usize {
    let base = base_args_no_viewers_v2(depth, n_in, n_out, withdraw_amount);
    let idx_n_viewers = base + 1; // appended right after the blacklist section
    idx_n_viewers + 1
}

fn viewer_fvk_commit_index_v2(
    depth: usize,
    n_in: usize,
    n_out: usize,
    withdraw_amount: u64,
    ...
    let per_viewer = 2 + 2 * n_out;
    viewer_start_index_v2(depth, n_in, n_out, withdraw_amount) + viewer * per_viewer
}

fn viewer_fvk_index_v2(
    depth: usize,
    n_in: usize,
    n_out: usize,
    withdraw_amount: u64,
    ...
    viewer_fvk_commit_index_v2(depth, n_in, n_out, withdraw_amount, viewer) + 1
}

fn private_indices_note_spend_v2(
    depth: usize,
    n_in: usize,
    n_out: usize,
    withdraw_amount: u64,
    ...
    // Inputs.
    let mut cur = 7usize; // 1-based index of value_in_0 (domain..n_in occupy 1..6)
    for _ in 0..n_in {
        idx.push(cur); // value_in
        idx.push(cur + 1); // rho_in
        idx.push(cur + 2); // sender_id_in
        idx.push(cur + 3); // pos
        for k in 0..depth {
            idx.push(cur + 4 + k); // siblings[k]
        }
        // nullifier is public at cur + 4 + depth
        cur += 5 + depth;
    }

    // withdraw_amount, withdraw_to, n_out are public.
    cur += 3;

    // Outputs.
    for _ in 0..n_out {
        idx.push(cur); // value_out
        idx.push(cur + 1); // rho_out
        idx.push(cur + 2); // pk_spend_out
        idx.push(cur + 3); // pk_ivk_out
                           // cm_out is public at cur + 4
        cur += 5;
    }

    // inv_enforce is private.
    idx.push(cur);
    cur += 1;
    ...
    for _ in 0..bl_checks {
        for i in 0..BL_BUCKET_SIZE {
            idx.push(cur + i); // bl_bucket_entries[i]
        }
        idx.push(cur + BL_BUCKET_SIZE); // bl_bucket_inv
        cur += BL_BUCKET_SIZE + 1;
        for i in 0..BL_DEPTH {
            idx.push(cur + i); // bl_siblings[i]
        }
        cur += BL_DEPTH;
    }

    // Viewer section: fvk is private, digests are public.
    let per_viewer = 2 + 2 * n_out;
    cur += 1; // n_viewers (public)
...
#[test]
fn test_viewer_fvk_is_private_index_v2() {
    // Parameters selected to exercise the transfer blacklist path and multi-output viewer section.
    let depth = 8usize;
    let n_in = 1usize;
    let n_out = 2usize;
    let withdraw_amount = 0u64; // TRANSFER => 2 blacklist checks
    let n_viewers = 3usize;

    let priv_idx = private_indices_note_spend_v2(depth, n_in, n_out, withdraw_amount, n_viewers);

    for v in 0..n_viewers {
        let idx_commit = viewer_fvk_commit_index_v2(depth, n_in, n_out, withdraw_amount, v);
        let idx_fvk = viewer_fvk_index_v2(depth, n_in, n_out, withdraw_amount, v);
        assert!(
            priv_idx.contains(&idx_fvk),
            "viewer fvk must be private (missing idx={idx_fvk})"
        );
        assert!(
            !priv_idx.contains(&idx_commit),
            "viewer fvk_commit must be public (unexpected private idx={idx_commit})"
        );
    }
}
...
impl MerkleTree {
    fn new(depth: u8) -> Self {
        ...
        for i in (0..depth as usize).rev() {
            zero_hashes[i] =
                poseidon2::mt_combine(i as u8, &zero_hashes[i + 1], &zero_hashes[i + 1]);
        }
        let size = 1 << depth;
        Self {
            depth,
...
fn test_valid_spend_one_output() -> Result<()> {
    ...
    let output1_cm =
        poseidon2::note_commitment(&domain, output1_value, &output1_rho, &output1_recipient);
    ...
    println!(
        "✓ Balance: {} = {} + {}",
        value, withdraw_amount, output1_value
    );
...
fn test_valid_spend_two_outputs() -> Result<()> {
    ...
    let output2_cm =
        poseidon2::note_commitment(&domain, output2_value, &output2_rho, &output2_recipient);
    ...
    println!(
        "✓ Balance: {} = {} + {} + {}",
        value, withdraw_amount, output1_value, output2_value
    );
...
fn test_argument_count_validation() {
    ...
    println!(
        "✓ Argument count with 0 outputs: {} (11 + {})",
        args0.len(),
        depth
    );
    ...
    println!(
        "✓ Argument count with 1 output:  {} (11 + {} + 4)",
        args1.len(),
        depth
    );
    ...
    println!(
        "✓ Argument count with 2 outputs: {} (11 + {} + 8)",
        args2.len(),
        depth
    );
...
fn test_nullifier_malleability_regression() -> Result<()> {
    ...
    assert!(
        witness2_valid,
        "BUG DEMONSTRATED: Witness 2 also satisfies old constraints with different nf_key!"
    );
    ...
    assert!(
        correct_sk_valid,
        "Correct spend_sk should satisfy fixed constraints"
    );

    // Wrong spend_sk does NOT satisfy fixed constraints
    let mut wrong_sk = spend_sk;
    wrong_sk[0] ^= 1;
    let nf_wrong = poseidon2::nullifier(
        &domain,
        &poseidon2::nf_key_from_sk(&domain, &wrong_sk),
        &rho,
    );
    ...
    assert!(
        !wrong_sk_valid,
        "Wrong spend_sk should NOT satisfy fixed constraints"
    );
...
fn test_arbitrary_address_fails_commitment() -> Result<()> {
    ...
    println!(
        "  Commitment with arbitrary recipient: {}",
        hx(&cm_with_arbitrary)
    );
    ...
    assert_ne!(
        derived_recipient, arbitrary_address,
        "Derived recipient should differ from arbitrary address"
    );

    // The commitment won't match
    let is_valid =
        output_commitment_valid(&domain, value, &rho, &arbitrary_address, &cm_with_arbitrary);
    println!("    commitment valid: {}", is_valid);
    assert!(
        !is_valid,
        "Arbitrary address used as pk_out should NOT produce matching commitment"
    );
    ...
    for i in 0..5 {
        ...
        let is_valid =
            output_commitment_valid(&domain, value, &rho, &random_pk, &cm_with_arbitrary);
        println!("    random_pk[{}] → valid: {}", i, is_valid);
        assert!(
            !is_valid,
            "Random pk_out should not produce matching commitment"
        );
    }
...
fn test_only_valid_privacy_addresses_can_receive() -> Result<()> {
    ...
    let circuit_computed_cm =
        poseidon2::note_commitment(&domain, value, &rho, &circuit_derived_recipient);
    ...
    assert_eq!(
        circuit_computed_cm, cm,
        "Circuit should accept valid pk_out"
    );
    ...
    assert_eq!(
        user_b_derived_address, user_b_address,
...
fn test_normal_chain_address_would_fail() -> Result<()> {
    ...
    println!(
        "  Normal chain address (simulated): {}",
        hx(&normal_chain_address)
    );
    ...
    println!(
        "    Using normal_address as pk_out → derived recipient: {}",
        hx(&derived)
    );
...
fn test_address_protection_full_flow() -> Result<()> {
    ...
    assert_eq!(
        alice_addr_check, alice_addr,
        "Alice's sk must derive to input recipient"
    );
    ...
    let output_cm_check =
        poseidon2::note_commitment(&domain, output_value, &output_rho, &bob_addr_derived);
    ...
    println!(
        ...
        input_value, withdraw, output_value
    );
    ...
    assert_eq!(
        bob_addr_from_sk, bob_addr,
        "Bob's sk derives to output recipient"
    );
    ...
    let evil_cm_check =
        poseidon2::note_commitment(&domain, output_value, &output_rho, &evil_derived);

    println!(
        "  Attacker tries arbitrary address: {}",
        &hx(&evil_addr)[..16]
    );
    println!("  Derived recipient: {}", &hx(&evil_derived)[..16]);
    println!("  Commitment match: {}", evil_cm_check == evil_cm);
    assert_ne!(
        evil_cm_check, evil_cm,
        "Arbitrary address should not produce valid commitment"
    );
